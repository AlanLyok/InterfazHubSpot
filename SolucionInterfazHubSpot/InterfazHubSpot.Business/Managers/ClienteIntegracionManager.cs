...
namespace InterfazHubSpot.Business.Managers
{
    public sealed class ClienteIntegracionManager
    {
        ...
        /// <summary>
        /// Persiste el ID de compañía HubSpot en el atributo variable CLIENTES.id_hubspot (no-op si ya coincide).
        /// </summary>
        public void GuardarIdHubSpotCliente(int clienteId, string idHubSpot)
        {
            ...
            using (var db = new MSGestionContext(_ctx))
            {
                ...
                try
                {
                    using (var cmd = conn.CreateCommand())
                    {
                        cmd.CommandText = "EXEC dbo.InterfazHubSpot_Cliente_GuardarIdHubSpot @ClienteId, @IdHubSpot";
                        cmd.Parameters.Add(new SqlParameter("@ClienteId", clienteId));
                        cmd.Parameters.Add(new SqlParameter("@IdHubSpot", idHubSpot.Trim()));
                        cmd.ExecuteNonQuery();
                    }
                }
                finally
