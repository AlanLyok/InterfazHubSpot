...
namespace InterfazHubSpot.Tests.Unit.HubSpot
{
    ...
    public sealed class HubSpotCrmClientEndpointsTests : IDisposable
    {
        ...
        [Fact]
        public async Task AssociateContactToCompanyAsync_Http500_LanzaHubSpotHttpRetriesExhaustedException()
        {
            var maxAttempts = _cfg.MaxHttpRetries + 1;
            ...
            Assert.IsType<HubSpotHttpRetriesExhaustedException>(ex);
            Assert.Equal(500, ((HubSpotHttpRetriesExhaustedException)ex).StatusCode);
            Assert.Equal(maxAttempts, _handler.Requests.Count);
        }
