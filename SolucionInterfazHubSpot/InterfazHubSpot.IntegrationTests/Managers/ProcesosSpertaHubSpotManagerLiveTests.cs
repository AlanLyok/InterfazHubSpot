...
namespace InterfazHubSpot.IntegrationTests.Managers
{
    ...
    public sealed class ProcesosSpertaHubSpotManagerLiveTests
    {
        ...
        public void ReclamarPendientes_Y_MarcarError_NoDuplicaFilasPorIdentificador()
        {
            // Anti-duplicado: ReclamarPendientes + MarcarError deben UPDATE la misma fila (ObjectState Modified).
            // Setup: INSERT fila Pendiente vía USER_POS_Clientes_Agregar o SQL directo; anotar ProcesoId e Identificador.
            // var countAntes = SELECT COUNT(*) FROM dbo.ProcesosSpertaHubSpot WHERE Identificador=@clienteId AND Destino='HubSpot';
            // var tomados = manager.ReclamarPendientes(IntegracionDestinos.HubSpot, 1);
            // manager.MarcarError(tomados[0].ProcesoId, "test");
            // var countDespues = SELECT COUNT(*) ... mismo filtro;
            // Assert.Equal(countAntes, countDespues);
            // Verificar: una sola fila; Estado=Error; Intentos incrementado solo en reclamo (no INSERT huérfanos).
        }
