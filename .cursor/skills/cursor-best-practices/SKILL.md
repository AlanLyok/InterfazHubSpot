...
### SpertaAPI monorepo (`2 Desarrollo`)

When working in **`SpertaAPI` / Cursor** alongside .NET Framework Web API hosts: enabling the MCP server **`mssql-spertaapi-dev`** exposes **`execute_sql`** against the project's dev SQL tier (configured under `services/mssql-mcp-server/.env`). Use it for ERP schema/data exploration coordinated with [.cursor/skills/dotnet-best-practices/SKILL.md](dotnet-best-practices/SKILL.md). Reference doc: **`docs/reference/mcp-mssql-spertaapi-dev.md`** from the repo root; how-to wiring: **`docs/how-to/mcp-mssql-desarrollo-cursor.md`**.
