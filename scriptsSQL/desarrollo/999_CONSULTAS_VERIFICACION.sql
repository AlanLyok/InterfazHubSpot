...
select * from ProcesosSpertaHubSpot order by ProcesoId desc
select * from ProcesosSpertaHubSpotLog order by ProcesoId desc
...
select nrodocumento, * from clientes where NroDocumento = '20-33403232-9'
select * from MSAtributosValores where registroid = 79

select nrodocumento, * from clientes where clienteid = 77
...
-- update clientes set nrodocumento = '30-50097401-6'  where clienteid = 94

-- update clientes set RazonSocial = 'TEST-GRANIX ASOCIACION ARGENTINA DE LOS ADVENTISTAS DEL SEPTIMO D' where clienteid = 77
